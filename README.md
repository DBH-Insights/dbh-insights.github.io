...
```
 ...
 │ export                   │           │ creds in Keychain   │         │          │
 └──────────────────────────┘           └─────────────────────┘         └──────────┘
```

Your vCenter passwords stay in your computer's keychain. They are never sent to the website, and the helper only reads from vCenter — it can't change anything.

## Getting started

1. [Install the helper](#install-the-helper) on your Mac.
...
**Requirements:** a Mac with Apple Silicon (M1 or later), and network access from that Mac to your vCenters.

1. Download the installer: [DBH Insights Helper_0.3.0_aarch64.dmg](helper-app/DBH%20Insights%20Helper_0.3.0_aarch64.dmg). On the file's GitHub page, click the **Download raw file** button.
...
The helper runs in the menu bar. Closing its window leaves it running; use the menu bar icon to reopen the settings or to quit. Quitting logs out of every vCenter session, and the website can't load data until you start the helper again.
...
Settings are saved in `~/Library/Application Support/dev.dbhinsights.helper/config.json`. Passwords are saved in the macOS Keychain under `dbh-insights-helper`.
...
- Passwords are stored in the macOS Keychain. They're never written to a file or sent to the website.
